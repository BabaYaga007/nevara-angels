       <!-- Header Area Start -->
       <header>
           <div class="container-fluid header-top-area header-sticky">
               <div class="row">
                    <!-- Logo Start -->
                    <div class="col-lg-2 col-5 col-full-xs" style="margin-top: -20px;">
                        <div class="logo">
                            <!-- <a class="primary-img" href="#"><img src="img/logo/logo.png" alt="brand-image" width="120px"></a> -->
                            <a class="sticky-logo" href="#"><img src="img/logo/logo1.png" alt="brand-image" width="120px"></a>
                        </div>
                    </div>
                    <!-- Logo End -->
                    <div class="col-12 xs-device d-none">
                        <ul class="search-form mobile-form">
                            <li>
                                <form action="#">
                                    <input type="text" class="search" name="search" placeholder="Search for products...">
                                </form>
                                <i class="pe-7s-search"></i>
                            </li>
                        </ul>
                    </div>
                    <!-- Primary-Menu Start -->
                    <div class="col-lg-7 d-none d-lg-block">
                        <div class="primary-menu">
                            <nav>
                                <ul class="primary-menu-list text-center">
                                    
                                    <li ><a href="index.php" class="hover-animation" >Home</a></li>

                                    <li><a href="#" class="hover-animation">Lamps<i class="pe-7s-angle-down "></i></a>
                                        <!-- Mega Menu Start -->
                                        <ul class="ht-dropdown">
                                            <!-- Single Mega Sub Menu Start -->
                                            <li>
                                                <!-- <h3>SOFAS & LOVESEATSs</h3> -->
                                                <ul>
                                                    <li><a href="categorie-page.php">Floor Lamps</a></li>
                                                    <li><a href="categorie-page.php">Pendant Lamps</a></li>
                                                    <li><a href="categorie-page.php">Table Lamps</a></li>
                                                    <li><a href="categorie-page.php">Wall Lamps</a></li>
                                                </ul>
                                            </li>
                                            
                                        </ul>
                                        <!-- Mega Menu End -->
                                    </li>
                                    <!-- Decor -->
                                    <li><a href="#" class="hover-animation" style="">Pages<i class="pe-7s-angle-down"></i></a>
                                        <!-- Home Version Dropdown Start -->
                                        <ul class="ht-dropdown">
                                            <li><a href="categorie-page.php">shop</a></li>
                                            <li><a href="product-page.php">Product Details</a></li>
                                            <li><a href="cart.php">cart</a></li>
                                            <li><a href="checkout.php">checkout</a></li>
                                            <li><a href="wish-list.php">wish list</a></li>
                                            <li><a href="blog.php">blog</a></li>
                                            <li><a href="blog-details.php">blog details</a></li>
                                            <li><a href="contact.php">contact</a></li>
                                            <li><a href="privacy.php">Privacy Policy</a></li>
                                            <li><a href="404.php">404</a></li>
                                        </ul>
                                        <!-- Home Version Dropdown End -->
                                    </li>
                                    <li><a href="about-us.php" class="hover-animation" style="">About us</a></li>
                                </ul>
                            </nav>
                        </div>
                    </div>
                    <!-- Primary-Menu End -->
                    <!-- Header All Shopping Selection Start -->
                    <div class="col-lg-3 col-7 col-full-xs">
                        <div class="main-selection">
                            <ul class="selection-list text-end">
                                <!-- Searcch Box Start -->
                                <li class="hidden-control"><i class="pe-7s-search"></i>
                                    <ul class="search-form ht-dropdown">
                                        <li>
                                            <form action="#">
                                                <input type="text" class="search" name="search" placeholder="Search for products...">
                                            </form>
                                            <i class="pe-7s-search"></i>
                                        </li>
                                    </ul>
                                </li>
                                <!-- Search Box End -->
                                <li><a href="wish-list.php"><i class="pe-7s-like"></i><span>2</span></a></li>
                                <li><i class="pe-7s-shopbag"></i><span>2</span>
                                    <ul class="ht-dropdown main-cart-box">
                                        <li>
                                            <!-- Cart Box Start -->
                                            <div class="single-cart-box">
                                                <div class="cart-img">
                                                    <a href="#"><img src="img/menu/1.webp" alt="cart-image"></a>
                                                </div>
                                                <div class="cart-content">
                                                    <h6><a href="#">Alpha Block Black Polo T-Shirt</a></h6>
                                                    <span>1 × $399.00</span>
                                                </div>
                                                <i class="pe-7s-close"></i>
                                            </div>
                                            <!-- Cart Box End -->
                                            <!-- Cart Box Start -->
                                            <div class="single-cart-box">
                                                <div class="cart-img">
                                                    <a href="#"><img src="img/menu/2.webp" alt="cart-image"></a>
                                                </div>
                                                <div class="cart-content">
                                                    <h6><a href="#">Red Printed Round Neck T-Shirt</a></h6>
                                                    <span>2 × $299.00</span>
                                                </div>
                                                <i class="pe-7s-close"></i>
                                            </div>
                                            <!-- Cart Box End -->
                                            <!-- Cart Footer Inner Start -->
                                            <div class="cart-footer fix">
                                                <h5>total :<span class="f-right">$698.00</span></h5>
                                                <div class="cart-actions">
                                                    <a class="checkout" href="checkout.php">Checkout</a>
                                                </div>
                                            </div>
                                            <!-- Cart Footer Inner End -->
                                        </li>
                                    </ul>
                                </li>
                                
                            </ul>
                        </div>
                    </div>
                    <!-- Header All Shopping Selection End -->
                    <!-- Mobile Menu  Start -->
                    <div class="mobile-menu d-block d-lg-none">
                        <nav>
                            <ul>
                                
                                <li><a href="index.php" style="">Home</a></li>

                                <li><a href="#" style="color: black;">Lamps</a>
                                    <!-- Mobile Menu Dropdown Start -->
                                    <!-- <ul>
                                        <li> -->
                                            <!-- <a href="#">sofas & loveseats</a> -->
                                            <!-- Mobile Menu Sub-Dropdown Start -->
                                            <ul>
                                                <li><a href="categorie-page.php">Floor Lamps</a></li>
                                                <li><a href="categorie-page.php">Pendant Lamps</a></li>
                                                <li><a href="categorie-page.php">Table Lamps</a></li>
                                                <li><a href="categorie-page.php">Wall Lamps</a></li>
                                            </ul>
                                            <!-- Mobile Menu Sub-Dropdown End -->
                                        <!-- </li>
                                        
                                    </ul> -->
                                    <!-- Mobile Menu Dropdown End -->
                                </li>
                                <!-- Decor  -->
                                <li><a href="#" style="color: black;">pages</a>
                                    <!-- Home Version Dropdown Start -->
                                    <ul>
                                        <li><a href="categorie-page.php">shop</a></li>
                                        <li><a href="product-page.php">Product Details</a></li>
                                        <li><a href="cart.php">cart</a></li>
                                        <li><a href="checkout.php">checkout</a></li>
                                        <li><a href="wish-list.php">wish list</a></li>
                                        <li><a href="blog.php">blog</a></li>
                                        <li><a href="blog-details.php">blog details</a></li>
                                        <li><a href="contact.php">contact</a></li>
                                        <li><a href="privacy.php">Privacy Policy</a></li>
                                        <li><a href="404.php">404</a></li>
                                    </ul>
                                    <!-- Home Version Dropdown End -->
                                </li>
                                <li><a href="about-us.php" style="color: black;">about us</a></li>
                            </ul>
                        </nav>
                    </div>
                    <!-- Mobile Menu  End -->
               </div>
           </div>
        </header>
        <!-- Header Area End -->